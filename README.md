...
Three defects, by three different routes, all of them mine. They are here
because a repository that only shows the working version is less useful than one
that shows what broke.

**A regex pattern that was wrong 49% of the time.** The library exposes three
pre-tokenization patterns. Only `cl100k_base` had ever been compared against the
reference implementation. When the differential suite was extended to the other
two, `o200k_base` disagreed on 490 of 1,000 generated inputs. The pattern had
been simplified by hand months earlier and never checked. All three are now
copied verbatim from `tiktoken._pat_str`, with the provenance in a comment.

**A use-after-free in the arena allocator.** The arena grew with `realloc`, which
is allowed to move the block. When it moved, every pointer it had handed out
became dangling, and the hash maps borrow their key bytes from the arena rather
than copying them, so both maps were left full of pointers into freed memory.
Every test passed, because `vocab_load_mem` pre-sizes the arena and the real
vocabulary fits, so the growth path almost never ran. A fuzzer found it in under
a minute. The arena is now a block list, and nothing it hands out ever moves.

**A performance number that had never been measured.** Chapter 3 quoted an
average of 1.37 probes on a hit at 70% load. Measuring it gave 2.18, and the
1.37 turned out to be roughly right for the load factor the real vocabulary sits
at, which is 38%, not 70%. `tools/measure_probes.c` exists so that the figure in
the book can be reproduced with one command rather than trusted.
...
Four kinds of test: unit tests over synthetic fixtures, an integration suite that compares this implementation against the official Python `tiktoken` package token for token, a differential suite that does the same on three thousand generated inputs per run, and two fuzzers. All four run in CI on every push.
...
`test_integration` compares ten strings a human chose. `test_reference_batch`
compares a thousand this implementation did not choose, against each of the
three encodings, which is a different job: curated cases check that the obvious
things work, generated ones find what nobody thought to write down.

- **1,000 generated inputs per encoding**, so 3,000 comparisons per run, against
  `cl100k_base`, `o200k_base` and `p50k_base`.
- **Six shapes**, because the `cl100k` pattern branches on exactly these
  distinctions: contractions, digit runs, whitespace, letters, punctuation and
  multi-byte UTF-8.
- **A fixed-seed generator**, so a failure can be reproduced exactly rather than
  described.
- **A hex-encoded batch protocol** between the C binary and the Python
  reference, because passing generated bytes through a shell means the shell gets
  a vote on the test data.
- **Curated code point ranges.** An earlier version generated unassigned code
  points, which different PCRE2 builds classify differently depending on their
  Unicode version, so 2 in 3,000 cases failed depending on the host rather than
  the code.

This is what found the `o200k_base` pattern. Ten hand-picked strings had passed
for months.

#### Fuzzing

Two harnesses under `fuzz/`, both built with libFuzzer plus AddressSanitizer and
UndefinedBehaviorSanitizer:

- **`fuzz_base64`** on the base64 decoder, which is the first thing that touches
  a vocabulary file.
- **`fuzz_vocab`** on the vocabulary parser, which is the only component that
  reads untrusted input of arbitrary length and shape.

`fuzz/corpus/` holds fourteen seeds, four of them named `regress-*`. Those four
are real crashes, frozen so they can never come back: arena growth, a duplicate
rank, a duplicate token, and a rank that is not a number. Each one exercises a
distinct path, so a regression in any of them fails the run rather than being
masked by the others.

One detail worth knowing if you fuzz your own code. The first run reached 30
million executions at coverage 4, which is not a clean result, it is random
noise: only the harness had been instrumented. The library needs building with
`-fsanitize=fuzzer-no-link` for the fuzzer to see inside it. With that flag the
same run reached 2,232 features immediately, and found the arena bug.
